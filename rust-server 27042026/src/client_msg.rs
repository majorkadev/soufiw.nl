...
pub fn parse(data: &[u8]) -> Result<ClientMsg> {
    ...
    match outer.msg_type {
        ...
        4 => {
            let inner = Table::parse(outer.payload)?;
            let entry_id = inner.read_u32(0).unwrap_or(0);
            Ok(ClientMsg::DuplicateEntry { entry_id })
        }
        3 => {
            let inner = Table::parse(outer.payload)?;
            let name = inner.read_string(0).unwrap_or_default();
            let content = inner.read_string(2).unwrap_or_default();
        ...
        2 => {
            let inner = Table::parse(outer.payload)?;
            let entry_id = inner.read_u32(0).unwrap_or(0);
            let delete_type = inner.read_u32(2).unwrap_or(0);
            Ok(ClientMsg::DeleteEntry { entry_id, delete_type })
        }
        1 => {
            let inner = Table::parse(outer.payload)?;
            let entry_id = inner.read_u32(0).unwrap_or(0);
        ...
        6 => {
            let inner = Table::parse(outer.payload)?;
            let entry_id = inner.read_u32(0).unwrap_or(0);
            Ok(ClientMsg::GenerateLink { entry_id })
        }
        7 => {
            let inner = Table::parse(outer.payload)?;
            let content = inner.read_string(0).unwrap_or_default();
            let style_entry_id = inner.read_u32(1).unwrap_or(0);

            Ok(ClientMsg::MenuConfigSave {
                style_entry_id,
                content,
            })
        }
