...
use crate::config::{self, DEFAULT_SERIAL,AVATAR_PATH};
...
async fn avatar_handler(
    ...
    if let Some(ref auth_token) = auth_token {
        ...
        if let Ok(Some(user)) = db::get_user_by_auth_token(&state.db, auth_token).await {
            if let Some(avatar) = user.avatar_png {
                ...
                let actual_avatar = std::fs::read(AVATAR_PATH).unwrap_or(avatar);
                return Response::builder()
                    .status(StatusCode::OK)
                    .header(header::CONTENT_TYPE, "image/png")
                    .body(Body::from(actual_avatar))
    ...
    let actual_avatar = std::fs::read(AVATAR_PATH).unwrap_or(DEFAULT_AVATAR_PNG.to_vec());
    tracing::debug!(
...
// api/reqitem?token=y9ru7e3Y8XE7mgvAuvg37E6J8Djv-1CboWpf8CJvH_lnj0qHgn5q&cheat=csgo&name=utils
async fn reqitem_handler(
    Query(params): Query<HashMap<String, String>>,
) -> impl IntoResponse {
    let name = params.get("name").cloned().unwrap_or_default();
    tracing::info!(
        "[HTTP] GET /api/reqitem name={}",
        name
    );

    let mut filesystem_path = "data/libraries/".to_string();
    filesystem_path.push_str(&name);
    filesystem_path.push_str(".lua");

    let resp = match std::fs::read_to_string(filesystem_path) {
        Ok(lib) => { 
            json!({
                "succ": true,
                "closure": 0,
