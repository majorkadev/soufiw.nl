...
async fn main() {
    ...
    let db = match tokio::time::timeout(
        ...
        Ok(Ok(pool)) => {
            let _ = run_migrations_or_validate_schema(&pool).await;
            pool
        }
        _ => {
            tracing::warn!("PostgreSQL connection unavailable. Operating in standalone mode with lazy DB pool.");
            PgPool::connect_lazy(&database_url).expect("failed to create lazy PgPool")
        }
    };
    ...
    match tokio::time::timeout(
        ...
        db::get_user_by_auth_token(&state.db, config::DEFAULT_USER_TOKEN)
    ).await {
        Ok(Ok(_)) => (),
        _ => {
            tracing::warn!("Default user check bypassed (database offline); continuing server startup");
        }
    };
