...
pub async fn delete_log_entry(pool: &PgPool, user_id: Uuid, entry_id: i32) -> Result<bool> {
...
pub async fn get_log_entry(pool: &PgPool, user_id: Uuid, entry_id: i32) -> Result<LogEntryRow> {
    let result = sqlx::query_as::<_, LogEntryRow>(
...
pub async fn get_entry_storage(pool: &PgPool, user_id: Uuid, entry_id: i32) -> Result<ScriptRow> {
...
pub async fn update_deletion_state(
    pool: &PgPool,
    user_id: Uuid,
    entry_id: i32,
    awaiting_deletion: i32,
) -> Result<bool> {
    let result = sqlx::query(
        "UPDATE log_entries SET awaiting_deletion = $3 WHERE user_id = $1 AND entry_id = $2",
    )
    .bind(user_id)
    .bind(entry_id)
    .bind(awaiting_deletion)
    .execute(pool)
    .await?;
    Ok(result.rows_affected() > 0)
}
